...
const Emails = sequelizeDb.Emails
const Op = sequelizeDb.Sequelize.Op

exports.create = async (req, res, next) => {
  try {
    const data = await Emails.create(req.body)
...
exports.findAll = async (req, res, next) => {
  try {
    ...
    const result = await Emails.findAndCountAll({
      where: condition,
...
exports.findOne = async (req, res, next) => {
  try {
    ...
    const data = await Emails.findByPk(id)
...
exports.update = async (req, res, next) => {
  try {
    const id = req.params.id
    const [numberRowsAffected] = await Emails.update(req.body, { where: { id } })
...
exports.delete = async (req, res, next) => {
  try {
    const id = req.params.id
    const numberRowsAffected = await Emails.destroy({ where: { id } })
