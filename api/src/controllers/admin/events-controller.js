...
const Events = sequelizeDb.Events
...
exports.findAll = async (req, res, next) => {
  try {
    ...
    const result = await Events.findAndCountAll({
      where: condition,
      attributes: ['id', 'promotorId', 'townId', 'spotId', 'categoryId', 'title', 'description', 'price', 'email', 'createdAt', 'updatedAt'],
      limit,
      offset,
...
exports.findOne = async (req, res, next) => {
  try {
    ...
    const data = await Events.findByPk(id)
...
exports.update = async (req, res, next) => {
  try {
    const id = req.params.id
    const [numberRowsAffected] = await Events.update(req.body, { where: { id } })
...
exports.delete = async (req, res, next) => {
  try {
    const id = req.params.id
    const numberRowsAffected = await Events.destroy({ where: { id } })
