...
exports.findAll = async (req, res, next) => {
  try {
    ...
    const count = await Subscription.countDocuments(whereStatement)
...
exports.findOne = async (req, res, next) => {
  try {
    ...
    const data = await Subscription.findById(id).lean().exec()
...
exports.update = async (req, res, next) => {
  try {
    const id = req.params.id
    const data = await Subscription.findByIdAndUpdate(id, req.body, { new: true }).lean().exec()
...
exports.delete = async (req, res, next) => {
  try {
    const id = req.params.id
    const data = await Subscription.findByIdAndUpdate(id, { deletedAt: new Date() })
