...
const Languages = sequelizeDb.Languages
const Op = sequelizeDb.Sequelize.Op

exports.create = async (req, res, next) => {
  try {
    const data = await Languages.create(req.body)
...
exports.findAll = async (req, res, next) => {
  try {
    ...
    const result = await Languages.findAndCountAll({
      where: condition,
      attributes: ['id', 'name', 'alias', 'selected', 'default', 'createdAt', 'updatedAt'],
      limit,
      offset,
...
exports.update = async (req, res, next) => {
  try {
    const id = req.params.id
    const [numberRowsAffected] = await Languages.update(req.body, { where: { id } })
...
exports.delete = async (req, res, next) => {
  try {
    const id = req.params.id
    const numberRowsAffected = await Languages.destroy({ where: { id } })
